...
class DemoARCore : ITest
{
	...
	LightMode  mode          = LightMode.Image;
    ...
    public UpdatePage UpdatePageMethod;

    public void Initialize()
	{
        arCoreSurface = Mesh.GeneratePlane(new Vec2(1, 1), 0, true);

        Renderer.EnableSky = true;
		LoadSkyImage("sky.jpg");
	...
	public void Step()
	{
		...
		//NOTE: Sets the CameraRoot as the root for the ARBackground so the plane is basically always in front of it. It's a bit more jank than fixing the sky material, but the SK sky material is very heavily intended to be a cubemap which ARCore is not, so this is the solution that avoids messing with SK code 
		Hierarchy.Push(Renderer.CameraRoot);
		if (arCoreMaterial != null)
		{
			arCoreSurface.Draw(arCoreMaterial, Matrix.R(new Vec3(90, 0, 0)) * Matrix.T(0, 0, 0));
		}
		Hierarchy.Pop();
	...
	public void LoadSkyImage(string file)
	{
